...
{
	while( (end > begin) && cmp(vp.val, arr[end-1].val) ) {
		end--;
	}
	arr[end] = vp;
	end++;
...
int solution( int K, int A[], int N )
{
	...
	for( i = 0; i < N; ++i ) {
		while( j < N ) {
			...
			mini_end = add_element( vp, mini, mini_begin, mini_end, less_equal );
			maxi_end = add_element( vp, maxi, maxi_begin, maxi_end, greater_equal );
